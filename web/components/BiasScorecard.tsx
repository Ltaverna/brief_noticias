...
function classifyCard(cell: Data["rows"][0]["cells"][string]): string {
  if (cell.total === 0) return "";
  const fav = cell.favorable / cell.total;
  const crit = cell.critico / cell.total;
  if (fav >= 0.5 && fav > crit + 0.2) return "border-l-4 border-l-emerald-400";
  if (crit >= 0.5 && crit > fav + 0.2) return "border-l-4 border-l-rose-400";
  return "";
}
    ...
    <>
      ...
      <div className="md:hidden space-y-4">
        {data.rows.map((row) => (
          ...
          >
            <h3 className="font-mono text-sm font-bold text-stone-700 dark:text-stone-300">
              {row.source}
            </h3>
            <ul className="mt-3 space-y-2">
              {data.entities.map((e) => {
                ...
                return (
                  <li
                    key={e.canonical}
                    className={`flex items-center justify-between gap-3 rounded px-2 py-1.5 text-sm ${classify(cell)} ${classifyCard(cell)}`}
                  >
